#!/bin/xsh
error InstallerAmd64Error = Failed(message: Str)
...
proc run_argv(target: Path, argv: List[Str], cwd: Path, envs: Record = {}) [process, error] {
  ...
  return Err(InstallerAmd64Error.Failed(f"${argv[0]} was signaled"))
}

proc main(...argv: List[Str]) [fs, process, env, error] {
  if argv.len() > 0 {
    return Err(InstallerAmd64Error.Failed("build-installer-amd64.xsh does not accept subcommands"))
  }
